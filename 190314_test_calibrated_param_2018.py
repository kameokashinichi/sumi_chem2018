...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 14:48:31 2019
...
"""
190314 test the availability of the calibrated parameter by using low-density data

variety -> SD1

calibration pattern
1. using koshihikari parameter -> "parameter_evaluation/2019-03-14_SD1_koshi/Summary.OUT"
2. using observed result -> "Low_density/JPRISD01.RIA"
3. using SD1_with_HWAM parameter -> "parameter_evaluation/2019-03-14_SD1_with_HWAH/Summary.OUT"
4. using SD1_without_HWAM parameter -> "parameter_evaluation/2019-03-14_SD1_without_HWAH/Summary.OUT"
"""


"""
"parameter_evaluation/2019-03-14_SD1_koshi_2018/Summary.OUT"(before calibration)
"""
...
record = []
with open(os.getcwd() + "/parameter_evaluation/2019-03-14_SD1_koshi_2018/Summary.OUT", 'r') as f:
    for row in f:
...
val = np.concatenate((val, sd1_2018["prefecture"].values.reshape(-1,1)), axis=1)
sim_df = pd.DataFrame(val, index=summary[:, 0].astype(np.int32), columns=["HWAM", "ADAT", "MDAT", "PDAT", "PREF"])

"""
"Low_density/JPRI1801.RIA" (measured value)
"""
record = []
with open(os.getcwd() + "/Low_density/JPRI1801.RIA", 'r') as f:
    for row in f:
        record.append(row.strip())
...
"""
"parameter_evaluation/2019-03-14_SD1_with_HWAH_2018/Summary.OUT" (calibrate including HWAM)
"""
record = []
with open(os.getcwd() + "/parameter_evaluation/2019-03-14_SD1_with_HWAH_2018/Summary.OUT", 'r') as f:
    for row in f:
        record.append(row.strip())
...
"""
"parameter_evaluation/2019-03-14_SD1_without_HWAH_2018/Summary.OUT" (calibrate without HWAM)
"""
record = []
with open(os.getcwd() + "/parameter_evaluation/2019-03-14_SD1_without_HWAH_2018/Summary.OUT", 'r') as f:
    for row in f:
        record.append(row.strip())
...
rmse_df.to_csv("190314_evaluate_calibration_via_RMSE_SD01_2018.csv")

"""
generate a histogram of the error value of each item

"df_lis" contains the dataframe of each combination
0 -> koshihikari:adat
1 -> without-hwam:adat
2 -> with-hwam:adat
3 -> koshihikari:mdat
...
for i in range(len(df_lis)):
    dif = abs(df_lis[i].iloc[:,0] - df_lis[i].iloc[:,1]).values
    dif_df = pd.DataFrame(dif, index=df_lis[i].index, columns=["DIFFERENCE"])
    df_lis[i] = pd.concat([df_lis[i], dif_df], axis=1)
    
#gen histogram of koshihikari:adat
fig = plt.figure(figsize=(10,6))
ax = fig.add_subplot(1,1,1)

ax.hist(df_lis[0].iloc[:,2].values, bins=15, cumulative=False, color="red")

plt.title("Histogram of the difference of "+df_lis[0].columns[0]+" VS OBSERVED -Anthesis Date-", fontsize=18)
plt.xlabel("the differences between simulated and observed", fontsize=16)
plt.ylabel("the number of simulations correspond to the value of x-axis", fontsize=16)
...
#gen histogram of without-hwam:adat
fig = plt.figure(figsize=(10,6))
ax = fig.add_subplot(1,1,1)

ax.hist(df_lis[1].iloc[:,2].values, bins=15, cumulative=False, color="red")

plt.title("Histogram of the difference of "+df_lis[1].columns[0]+" VS OBSERVED -Anthesis Date-", fontsize=18)
plt.xlabel("the differences between simulated and observed", fontsize=16)
plt.ylabel("the number of simulations", fontsize=16)
plt.setp(ax.get_xticklabels(), fontsize=14, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=14, visible=True)
...
#gen histogram of with-hwam:adat
fig = plt.figure(figsize=(10,6))
ax = fig.add_subplot(1,1,1)

ax.hist(df_lis[2].iloc[:,2].values, bins=15, cumulative=False, color="red")

plt.title("Histogram of the difference of "+df_lis[2].columns[0]+" VS OBSERVED -Anthesis Date-", fontsize=18)
plt.xlabel("the differences between simulated and observed", fontsize=16)
plt.ylabel("the number of simulations", fontsize=16)
plt.setp(ax.get_xticklabels(), fontsize=14, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=14, visible=True)
...
ax.hist(df_lis[6].iloc[:,2].values, bins=15, cumulative=False, color="red")

plt.title("Histogram of the difference of "+df_lis[6].columns[0]+" VS OBSERVED -Yield-", fontsize=18)
plt.xlabel("the differences between simulated and observed", fontsize=16)
plt.ylabel("the number of simulations", fontsize=16)
plt.setp(ax.get_xticklabels(), fontsize=14, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=14, visible=True)
...
plt.title("Comparison of maturity date among observed, koshi_param and SD1_param", fontsize=18)
plt.xlabel("field name", fontsize=15)
plt.ylabel("Maturity date", fontsize=15)
plt.ylim([min(mdat2.iloc[:, 1]-10), max(mdat2.iloc[:, 0]+10)])
ax.set_yticklabels(doylist2Date(ax.get_yticks() -2018000))
plt.setp(ax.get_yticklabels(), fontsize=14, rotation=45, visible=True)
plt.savefig("190313_rice_gencalc_png/190316_MDAT_validation_SD1.png", bbox_inches="tight")
